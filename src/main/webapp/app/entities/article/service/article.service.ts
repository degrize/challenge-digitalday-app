...
type RestOf<T extends IArticle | NewArticle> = Omit<T, 'dateExpiration' | 'dateFabrication'> & {
...
export class ArticleService {
  ...
  protected convertDateFromClient<T extends IArticle | NewArticle | PartialUpdateArticle>(article: T): RestOf<T> {
    return {
      ...article,
      dateExpiration: article.dateExpiration?.format(DATE_FORMAT) ?? null,
      dateFabrication: article.dateFabrication?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restArticle: RestArticle): IArticle {
    return {
      ...restArticle,
      dateExpiration: restArticle.dateExpiration ? dayjs(restArticle.dateExpiration) : undefined,
      dateFabrication: restArticle.dateFabrication ? dayjs(restArticle.dateFabrication) : undefined,
    };
  }
